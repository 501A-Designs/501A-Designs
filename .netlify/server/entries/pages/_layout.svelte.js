import { c as create_ssr_component, a as subscribe, e as escape, b as add_attribute, d as each, f as spread, g as escape_object, v as validate_component } from "../../chunks/index2.js";
import { t as theme, l as language, c as currentRoute } from "../../chunks/store.js";
const app = "";
const Footer_svelte_svelte_type_style_lang = "";
const css = {
  code: "footer.svelte-12ux6ts.svelte-12ux6ts{max-height:55px;margin:0;transition:0.5s;user-select:none;white-space:nowrap;border:none;border-top:1px solid transparent;border-image:linear-gradient(90deg, var(--backgroundColor) 20%, var(--accentColor1) 50%, var(--backgroundColor) 80%);border-image-slice:1;background:linear-gradient(90deg,var(--backgroundColor) 25%,var(--accentColor0),var(--backgroundColor) 75%)}footer.svelte-12ux6ts div.svelte-12ux6ts{display:flex;align-items:center;gap:0.5em}",
  map: null
};
const Footer = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<footer class="svelte-12ux6ts">${slots.default ? slots.default({}) : ``}
	<div class="svelte-12ux6ts"><select><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select>
		<select><option value="english">English</option><option value="japanese">日本語</option></select></div>
</footer>`;
});
...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  $$unsubscribe_theme();
  $$unsubscribe_language();
  return `${$$result.head += `<!-- HEAD_svelte-168i5jk_START --><meta name="color-scheme"${add_attribute("content", $theme == "system" ? "light dark" : $theme, 0)}><link rel="stylesheet"${add_attribute("href", `/theme/${$theme}.css`, 0)}><!-- HEAD_svelte-168i5jk_END -->`, ""}
...
${validate_component(Footer, "Footer").$$render($$result, {}, {}, {
    default: () => {
      return `${slug !== "index" ? `<div><a href="/">${escape($language === "japanese" ? "メイン" : "Main")}</a>
			${typeof slug === "number" ? `<span>/ 
					<a href="/projects">${escape($language === "japanese" ? "創作物" : "Projects")}</a>
					 / ${escape($language === "japanese" ? "通し番号" : "Index")} #${escape(slug)}</span>` : `<span>/ 
					${slug === "about" ? `${escape($language === "japanese" ? "自分について" : "About Me")}` : ``}
					${slug === "projects" ? `${escape($language === "japanese" ? "創作物" : "Projects")}` : ``}
					${slug === "memo" ? `${escape($language === "japanese" ? "メモ" : "Memo")}` : ``}
					${slug === "card" ? `${escape($language === "japanese" ? "名刺" : "Card")}` : ``}</span>`}</div>` : `<span style="color: var(--textColor0)">501A Designs</span>`}`;
    }
  })}`;
});
export {
